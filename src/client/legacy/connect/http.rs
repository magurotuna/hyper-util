...
impl HttpConnector {
    ...
    ///
    /// This uses [`GaiResolver`] for DNS resolution and [`TokioTimer`](crate::rt::TokioTimer)
    /// for timeouts internally. Use [`HttpConnector::new_with_resolver_and_timer`] to provide
    /// a custom resolver and timer.
...
impl<R> HttpConnector<R> {
    ...
    ///
    /// This uses [`TokioTimer`](crate::rt::TokioTimer) for timeouts internally.
    /// Use [`HttpConnector::new_with_resolver_and_timer`] to provide a custom timer.
        ...
        for &(hosts, family, timeout, needs_ipv6_access) in scenarios {
            ...
            let (start, stream) = rt
                .block_on(async move {
                    ...
                    let connecting_tcp =
                        ConnectingTcp::new(dns::SocketAddrs::new(addrs), &cfg, &timer);
